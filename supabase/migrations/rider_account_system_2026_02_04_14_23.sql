...
-- Admin policies (admins can access all rider data)
CREATE POLICY "Admins can manage all riders" ON public.riders_2026_02_04_14_23
    FOR ALL USING (auth.role() = 'authenticated' AND EXISTS (
        SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin', 'manager')
    ));

CREATE POLICY "Admins can manage all rider tasks" ON public.rider_tasks_2026_02_04_14_23
    FOR ALL USING (auth.role() = 'authenticated' AND EXISTS (
        SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin', 'manager')
    ));

CREATE POLICY "Admins can manage all rider transactions" ON public.rider_transactions_2026_02_04_14_23
    FOR ALL USING (auth.role() = 'authenticated' AND EXISTS (
        SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin', 'manager')
    ));

CREATE POLICY "Admins can view all rider locations" ON public.rider_locations_2026_02_04_14_23
    FOR SELECT USING (auth.role() = 'authenticated' AND EXISTS (
        SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin', 'manager')
    ));

CREATE POLICY "Admins can manage all rider notifications" ON public.rider_notifications_2026_02_04_14_23
    FOR ALL USING (auth.role() = 'authenticated' AND EXISTS (
        SELECT 1 FROM public.profiles WHERE user_id = auth.uid() AND role IN ('super_admin', 'admin', 'manager')
    ));