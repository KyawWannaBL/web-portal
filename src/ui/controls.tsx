...
export function cx(...parts: Array<string | false | undefined>) { return parts.filter(Boolean).join(" "); }
...
export function AppButton(props: BtnLink | BtnButton) {
  ...
  return (
    <button type="button" className={cx("btn", props.className)} onClick={props.onClick} disabled={props.disabled}>
      {props.children}
    </button>
  );
}
